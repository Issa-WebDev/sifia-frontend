  ...
  useEffect(() => {
    const verifyPayment = async () => {
      try {
        ...
        let url;
        if (registrationId) {
          url = `${import.meta.env.VITE_BACKEND_URL}/api/payment/verify?registration_id=${registrationId}`;
        } else if (transactionId) {
          url = `${import.meta.env.VITE_BACKEND_URL}/api/payment/verify?transaction_id=${transactionId}`;
        } else {
